...
use zugferd::{
    ApplicableTradeTax, BilledQuantity, CountryCode, CurrencyCode, InvoiceBuilder, InvoiceTypeCode,
    NetPriceProductTradePrice, SpecificationLevel, SpecifiedTradeSettlementLineMonetarySummation,
};
// For Basic specification
use zugferd::{
    components::enums::identifier_scheme_code::IdentifierSchemeCode,
    AssociatedDocumentLineDocument, GlobalID, IncludedSupplyChainTradeLineItem,
    SpecifiedLineTradeAgreement, SpecifiedLineTradeDelivery, SpecifiedLineTradeSettlement,
...
fn main() {
    ...
    //Basic WL
    invoice_builder.set_invoice_notes(vec!["Note 1", "Very important note 2"]);
    ...
    invoice_builder.set_occurrence_date(chrono::NaiveDate::from_ymd_opt(2024, 7, 6).unwrap());
    ...
    invoice_builder.set_specified_trade_payment_terms_due_date(
